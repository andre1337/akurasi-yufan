...
class PDF extends FPDF{
  function Header(){
    ...
    $this->Cell(20,0,'Data klasifikasi',0,0,'L');
    $this->Ln();
    $this->Cell(5,1,'Laporan data klasifikasi',0,0,'L');
    $this->Ln();
...
$arr=getData($conn,$sql);
  ...
  $cell[$i][0]=$d["id"];
  $cell[$i][1]=$d["komplain"];
  $cell[$i][2]=$d["klasifikasi"];
  $cell[$i][3]=$d["normalisasi"];
  $i++;
}
...
$pdf->Cell(7,1,'komplain','LR',0,'C',1);
$pdf->Cell(5,1,'klasifikasi','LR',0,'C',1);
$pdf->Cell(3,1,'normalisasi','LR',0,'C',1);
$pdf->Ln();
$pdf->SetFont('Arial','','8');
...
for ($j=0;$j<$i;$j++){
  ...
  $pdf->Cell(7,1,$cell[$j][1],'B',0,'L'); // komplain
  $pdf->Cell(5,1,$cell[$j][2],'B',0,'L'); // klasifikasi
  $pdf->Cell(3,1,$cell[$j][3],'B',0,'L'); // normalisasi
  $pdf->Ln();
}
$pdf->Output();
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
