...
class PDF extends FPDF{
  function Header(){
    ...
    $this->Cell(20,0,'Data tweet',0,0,'L');
    $this->Ln();
    $this->Cell(5,1,'Laporan data tweet',0,0,'L');
    $this->Ln();
...
$pdf->Cell(2,1,'id_tweet','LR',0,'C',1);
$pdf->Cell(7,1,'tweet','LR',0,'C',1);
$pdf->Cell(5,1,'normalisasi','LR',0,'C',1);
$pdf->Cell(3,1,'kategori','LR',0,'C',1);
$pdf->Cell(9,1,'keterangan','LR',0,'C',1);
$pdf->Cell(1,1,'hastag','LR',0,'C',1);
...
for ($j=0;$j<$i;$j++){
  ...
  $pdf->Cell(2,1,$cell[$j][0],'B',0,'L'); // id_tweet
  $pdf->Cell(7,1,$cell[$j][1],'B',0,'L'); // tweet
  $pdf->Cell(5,1,$cell[$j][2],'B',0,'L'); // normalisasi
  $pdf->Cell(3,1,$cell[$j][3],'B',0,'L'); // kategori
  $pdf->Cell(9,1,$cell[$j][4],'B',0,'L'); // keterangan
  $pdf->Cell(1,1,$cell[$j][5],'B',0,'L'); // hastag
  $pdf->Cell(7,1,$cell[$j][6],'B',0,'L'); // 
  $pdf->Ln();
}
$pdf->Output();
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
