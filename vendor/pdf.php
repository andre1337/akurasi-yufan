...
class PDF extends FPDF{
  function Header(){
    ...
    $this->Cell(20,0,'Data vendor',0,0,'L');
    $this->Ln();
    $this->Cell(5,1,'Laporan data vendor',0,0,'L');
    $this->Ln();
...
$arr=getData($conn,$sql);
		foreach($arr as $d) {	
  $cell[$i][0]=$d["id_vendor"];
  $cell[$i][1]=$d["nama_vendor"];
  $cell[$i][2]=$d["telepon"];
  $cell[$i][3]=$d["email"];
  $cell[$i][3]=$d["alamat"];
  $cell[$i][3]=$d["username"];
...
$pdf->Cell(2,1,'id_vendor','LR',0,'C',1);
$pdf->Cell(7,1,'nama_vendor','LR',0,'C',1);
...
for ($j=0;$j<$i;$j++){
  ...
  $pdf->Cell(2,1,$cell[$j][0],'B',0,'L'); // id_vendor
  $pdf->Cell(7,1,$cell[$j][1],'B',0,'L'); // nama_vendor
  $pdf->Cell(5,1,$cell[$j][2],'B',0,'L'); // telepon
  $pdf->Cell(3,1,$cell[$j][3],'B',0,'L'); // email
  $pdf->Cell(3,1,$cell[$j][3],'B',0,'L'); // alamat
  $pdf->Cell(3,1,$cell[$j][3],'B',0,'L'); // username
