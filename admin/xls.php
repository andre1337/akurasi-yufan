...
if ($conn->connect_error) {
  trigger_error('Database connection failed: '  . $conn->connect_error, E_USER_ERROR);
    ...
    $buffer = "id_admin".$separator ."username".$separator ."password".$separator ."telepon".$separator ."email".$separator ."status".$separator ."gambar".$separator; 
    $buffer .= $newline; 
    
  $sql="select `id_admin`,`username`,`password`,`telepon`,`email`,`status`,`gambar` from `$tbadmin` order by `id_admin` desc";
  $jum=getJum($conn,$sql);	
  if($jum>0){						
